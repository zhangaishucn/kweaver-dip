...
<template>
  <div style="padding: 8px; box-sizing: border-box;">
    <div style="font-size: 13px; color: #000; line-height: 19px; margin: 0 0 9px 0;">{{$t('menu.deptAuditor')}}：</div>
    <el-table :data="deptTableData"  :height="tableHeight" ref="multipleTable" tooltip-effect="light" class="table-ellip" style="border-bottom: 0px">
        <el-table-column :label="$t('deptAuditorRule.deptName')">
          <template slot-scope="scope">
            <span :title="scope.row.name">{{ scope.row.name }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="auditorNames" :label="$t('deptAuditorRule.deptAuditor') ">
          <template slot-scope="scope">
            <workflowPopover placement="bottom-start" popper-class="dept-view" :title="auditorNamesPopover?$t('strategy.table.auditor'):''"  width="200" @show="auditorNamesPopover = true" trigger="click">
              <div class="popo-list" v-if="auditorNamesPopover">
                <div class="audit-list">
                  <template v-for="(item, index) in getAuditorNameList(scope.row)">
                    <div v-title :title="item" :key="index" style="width: 100%;overflow:hidden;text-overflow:ellipsis;white-space:pre;word-break:keep-all;"> {{item}}</div>
                  </template>
                </div>
              </div>
