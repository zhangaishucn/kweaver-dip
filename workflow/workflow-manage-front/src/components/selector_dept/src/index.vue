<template>
  <el-dialog :title="dialog_title" :close-on-click-modal="false" top="35px" :visible="visible" :append-to-body="true" custom-class="new-dialog" @close="cancel">
        ...
        <div class="choose-ul">
          <div class="head">
            ...
            <div class="right">
              ...
              <el-button v-if="check_data.length==0" type="text" disabled>{{ $t('modeler.common.clear') }}</el-button>
              <el-button v-else type="text" @click="delAllData">{{ $t('modeler.common.clear') }}</el-button>
            </div>
          </div>
          <ul style="height: 450px;margin-top: 8px">
           <el-tooltip v-for="(item, index) in check_data" :key="index" class="item" effect="light" :content="`${item.name}`" placement="top">
            <li>
              <span>
                {{ item.name }}
                <i class="el-icon-close" @click="delAppointData(item)" />
              </span>
            </li>
            </el-tooltip>
          </ul>
    ...
    <div slot="footer" class="dialog-footer">
      <div>
        <el-button type="primary" style="min-width: 80px" size="small" :disabled="check_data.length === 0" @click="confirm">{{ $t('button.confirm') }}</el-button>
        <el-button class="el-button-gray" style="min-width: 80px" size="small" @click="cancel">{{ $t('button.cancel') }}</el-button>
      </div>
    </div>
  </el-dialog>
</template>
