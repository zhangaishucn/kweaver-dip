...
def main(display_data):
    ...
    # 用户'iberly'申请‘市大数据中心’的‘物流明细表’的查询权限
    apply_title = f"{applicant_cn_type}'{applicant_name}'申请‘{datasource_name}’的‘{data_view_name}’的{operations_cn_name} 权限"
    if len(apply_title) > 200:
        apply_title = f"{applicant_name}:{data_view_name}"
    
    return applicant_name, applicant_cn_type, datasource_name, data_view_name, operations_cn_name, expiration, apply_title