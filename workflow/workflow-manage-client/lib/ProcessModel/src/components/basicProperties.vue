<template>
  <div class="basicProperties no-border-bottom">
    ...
    <div class="no-border">
      <div class="card-body">
        <el-form ref="basicForm"
                 ...
                 :class="elementType === 'Root' ? 'new-form' : ''"
          ...
          <template v-else-if="elementType === 'Task'">
            ...
            <el-form-item v-if="!is_share">
              ...
              <el-select v-model="deal_type"
                         style="width: 100%"
                         :disabled="is_edit&&disable">
                <el-option :label="$t('modeler.dealType.tjsh')"
                           value="tjsh" />
                <el-option :label="$t('modeler.dealType.hqsh')"
                           value="hqsh" />
                <el-option :label="$t('modeler.dealType.zjsh')"
                           value="zjsh" />
              </el-select>
            </el-form-item>
          </template>
        ...
        <div>
          <el-row :gutter="20">
            <el-col :span="8"
              ...
              <el-button type="primary"
                         size="mini"
                         icon="el-icon-plus"
                         @click="openStepSelector">{{$t('strategy.addStrategyName')}}</el-button>
              <el-button v-if="showHandelBtn"
                         size="mini"
                         icon="el-icon-edit-outline"
                         @click="editStepAll">{{$t('button.edit')}}</el-button>
              <el-button v-if="showHandelBtn"
                         size="mini"
                         icon="el-icon-delete"
                         @click="delStepAll">{{$t('button.delete')}}</el-button>
            </el-col>
            <el-col :span="16"
                    class="align-right">
              <MultiChoice v-model="search_obj"
          ...
          <div style="margin-top: 5px"
            ...
            <el-table ref="multipleTable"
              ...
              <el-table-column :label="$t('strategy.table.labName')"
                               prop="doc_name"
                               show-overflow-tooltip
                               width="300">
              </el-table-column>
              <el-table-column prop="auditorNames"
                  ...
                  <el-popover placement="bottom-start"
                    ...
                    <span slot="reference"
                      ...
                      <el-tooltip placement="top-start"
                                  ...
                                  :offset="0"
                                  :visible-arrow="false"
                                  :content="delAuditorNames(scope.row.audit_model, scope.row.auditorNames)">
                        <span style="overflow:hidden;text-overflow:ellipsis;white-space:pre;word-break:keep-all;"
                              @click="auditorNamesPopover = true">{{delAuditorNames(scope.row.audit_model, scope.row.auditorNames)}}</span>
                      </el-tooltip>
                    </span>
                  </el-popover>
              ...
              <el-table-column :label="$t('strategy.table.labType')"
                               width="210"
                               column-key="doc_type"
              ...
              <el-table-column :label="$t('strategy.table.auditModel')"
                               width="210">
                <template slot-scope="scope">
              ...
              <el-table-column :label="$t('strategy.table.hander')"
                               width="100">
                <template slot-scope="scope">
                  <el-button size="mini"
                             icon="el-icon-edit-outline"
          ...
          <div v-if="process_obj_type === '1'">
            <div class="rzsj_table_box margin-bottom-10">
              ...
              <div class="cell align-right"
                   style="width: 130px">
                <el-button size="mini"
                           @click="openFullSelector">{{ $t('modeler.common.add') }}</el-button>
                <el-button size="mini"
                           @click="cleanFullSelected()">{{ $t('modeler.common.clear') }}</el-button>
              </div>
            </div>
            <div class="revie-list"
              ...
              <template v-if="select_id_obj_list.length > 0">
                <div class="list"
                     v-for="(item, index) in select_id_obj_list"
                     :key="item.id">
                  <div class="cell-text">
                    <el-tooltip effect="light"
                                class="item"
                                :content="showTips(item)"
                                placement="top">
                      <div class="name"
                  ...
                  <div class="cell-btn">
                    <a class="btn"
                       @click="remove(item.id, 'user')"><i class="el-icon-error"></i></a>
                  </div>
                </div>
              </template>
              <template v-if="select_role_id_obj_list.length > 0">
                <div class="list"
                     v-for="(item, index) in select_role_id_obj_list"
                     :key="item.id">
                  <div class="cell-text">
                    <el-tooltip effect="light"
                                class="item"
                  ...
                  <div class="cell-btn">
                    <a class="btn"
                       @click="remove(item.id, 'role')"><i class="el-icon-error"></i></a>
                  </div>
                </div>
              </template>
              <div v-if="select_id_obj_list.length === 0 && select_role_id_obj_list.length === 0"
                   class="align-center gray">
    ...
    <OrgSelect ref="orgSelect"
               @output="fullSelectorCall"
               :deal_type="deal_type"
               :title="dialog_title"
               :multiple="true"></OrgSelect>
    <ShareSelect ref="stepSelect"
                 @output="stepSelectorCall"
                 :deal_type="deal_type"
                 :title="dialog_title"
                 :checkedUserIds="auditorObj"
                 :selectDocList="selectDocObj"
                 :procDefId="process_obj.id"></ShareSelect>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    delStepAll () {
      let _this = this
      _this.$dialog_confirm(_this.$i18n.tc('message.confirmDelete'), '', _this.$i18n.tc('button.confirm'), _this.$i18n.tc('button.confirm'), true).then(() => {
        let idArr = []
