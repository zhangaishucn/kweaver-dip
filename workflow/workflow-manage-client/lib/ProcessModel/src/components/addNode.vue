<template>
  <div class="add-node-btn-box">
    <div class="add-node-btn">
      <workflowPopover v-model="visible" placement="right-start" :disabled="addNodeDisabled || nodeList.length > 10">
  ...
  methods: {
    addType(type) {
      ...
      if(_this.nodeList.length > 10){
        _this.$dialog_confirm(_this.$i18n.tc('processCenter.auditorRestrictTips'), '', _this.$i18n.tc('button.confirm'), _this.$i18n.tc('button.cancel'), false ).then(() => {
        }).catch(() => {})
        return
      }
      if (type === 1) {
