  ...
  <div class="gjsz-box">
    <div class="sh-list padding-bottom-10">
      <div class="check">
        ...
        <div>
          <el-checkbox v-model="renameCheckbox" size="small" class="check" @change="enabledChange">{{ $store.state.app.secret.status === 'y' ? $t('modeler.secretRenameAudit') : $t('modeler.renameAudit')}}</el-checkbox>
        </div>
        <div class="gray-text">{{ $store.state.app.secret.status === 'y' ? $t('modeler.secretRenameCaution') : $t('modeler.renameCaution')}}</div>
      </div>
    </div>
    <div class="sh-list padding-bottom-10">
...
export default {
  ...
  methods: {
    initCheckbox(){
      ...
      if(_this.process_obj.advancedSetup.rename_switch === 'y'){
