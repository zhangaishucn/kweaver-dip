...
/**
 * @description 发起文档审核记录日志
 * @author ouandyang
 */
@Component(value = OperationLogConstants.DOC_AUDIT_ADD_LOG)
public class DocAuditApplyLogHandler extends AbstractLogHandler implements LogHandler {
    ...
    public OperationLogDTO buildLogMsg(Object[] args) {
        ...
        DocAuditLogService docAuditBizService = ApplicationContextHolder.getBean(DocConstants.DOC_AUDIT_LOG_PRFIX + serviceSuffix,
                DocAuditLogService.class);
        OperationLogDTO log = new OperationLogDTO();
        log.setMsg(docAuditBizService.buildApplyLogMsg(docAuditApplyModel));
        log.setExMsg(docAuditBizService.buildApplyLogExMsg(docAuditApplyModel));
        log.setLogType(ncTLogType.NCT_LT_OPEARTION);
        log.setOpType(ncTDocOperType.NCT_DOT_AUDIT_MGM.getValue());
        log.setUserId(docAuditApplyModel.getApplyUserId());
        try {
            log.setIp(Inet4Address.getLocalHost().getHostAddress());
