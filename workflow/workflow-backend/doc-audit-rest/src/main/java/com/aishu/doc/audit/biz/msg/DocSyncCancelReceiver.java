 ...
 */
@Slf4j
@Component(value = NsqConstants.CORE_AUDIT_SYNC_CANCEL)
public class DocSyncCancelReceiver implements MessageHandler, MessageHandleExecutor {

    /**
     * 流程作废原因,文件变更
     */
    private static final String CANCEL_REASON = "sync_cancel";
    /**
     * 操作用户-管理员（固定id）
     */
    ...
    public void handler(java.lang.String msg) {
        ...
        inBoxService.addInBoxMessage(NsqConstants.CORE_AUDIT_SYNC_CANCEL, msg); 
    }

    @Override
    public void handleMessage(String msg) {
        try {
            log.info("nsq===接收到取消同步申请消息：{}", msg);
            ...
            log.warn("nsq===处理取消同步申请消息异常, json解析失败！{message：{}}", new String(msg), e);
        } catch (NullPointerException e) {
            log.warn("nsq===处理取消同步申请消息异常, 业务数据不存在！{message：{}}", new String(msg), e);
        } catch (Exception e) {
            SysLogUtils.insertSysLog(SysLogBean.TYPE_ERROR, NsqConstants.CORE_AUDIT_SHARE_CANCEL, e, msg);
            log.warn("nsq===处理取消同步申请消息异常！{message：{}}", new String(msg), e);
            throw e;
        }finally {
		}
    }
