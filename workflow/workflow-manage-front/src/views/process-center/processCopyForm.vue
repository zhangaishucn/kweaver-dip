...
<template>
  <div>
    <el-dialog :title="$t('processCenter.copyProcess')" :visible="visible" :close-on-click-modal="false" :append-to-body="true" @close="close" v-dialogDrag custom-class="new-dialog" width="420px" :destroy-on-close="true">
      <el-form ref="processForm" label-position="left" :model="current_proc_obj" :label-width="$i18n.locale === 'en-us'?'145px':'86px'" size="small" :rules="rules">
        <el-form-item :label="$t('modeler.processName') + ':'" prop="name">
          <el-input
            v-model.trim="current_proc_obj.name"
            show-word-limit
