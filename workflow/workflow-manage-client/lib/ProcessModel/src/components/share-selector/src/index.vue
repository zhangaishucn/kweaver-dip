<template>
  <div>
    <el-dialog :title="title"
               ...
               width="628px">
      <div class="el-steps-2"
           v-if="!isUpdate">
        <el-steps :active="active_step"
                  simple>
          <el-step :title="$t('modeler.selectorStepOne')"
                   icon="el-icon-user"></el-step>
          <el-step :title="$t('modeler.selectorStepTwo')"
                   icon="el-icon-monitor"></el-step>
        </el-steps>
      </div>
      <div v-if="visible && active_step === 0">
        <div class="sers-2">
          <span class="clum"
                :style="$i18n.locale === 'en-us'?'width: 140px':''">{{$t('modeler.auditMode')}}：</span>
          <el-select v-model="audit_model">
            <el-option value="tjsh"
                       :label="$t('modeler.dealTypeTips.tjsh')"></el-option>
            <el-option value="hqsh"
                       :label="$t('modeler.dealTypeTips.hqsh')"></el-option>
            <el-option value="zjsh"
                       :label="$t('modeler.dealTypeTips.zjsh')"></el-option>
          </el-select>
        </div>
                  ...
                  <template slot-scope="{ item }">
                    <span v-if="item.parent_dep_paths[0] !== ''">
                      <el-tooltip class="item"
                        ...
                        <div>
                          <p style="overflow:hidden;text-overflow:ellipsis;white-space:pre;word-break:keep-all;">
                            <i class="icon iconfont icon-yonghu"></i>
                            {{ item.name }}
                          </p>
                          <div style="overflow:hidden;text-overflow:ellipsis;white-space:pre;word-break:keep-all;">{{ item.parent_dep_paths[0] }}</div>
                        </div>
                      </el-tooltip>
                    </span>
              ...
              <el-tree v-if="user_show_tree"
                ...
                <span class="custom-tree-node"
                  ...
                  <template v-if="data.type === 'loadMore'">
                    <div style="padding-left: 20px;color:#40a9ff;"
                         class="loadMoreLoading"
                         v-loading="loadMoreLoading">
                      <span>{{ node.label }} <i class="el-icon-d-arrow-right"
                           style="transform: rotate(90deg);-ms-transform: rotate(90deg);-moz-transform: rotate(90deg);-webkit-transform: rotate(90deg);-o-transform: rotate(90deg);"></i></span>
                    </div>
                  </template>
                  <template v-else>
          ...
          <div class="cell no-border">
            <div class="choose-ul">
              <div class="head">
                ...
                <div class="right">
                  <el-button type="text"
                             v-if="check_user_data.length==0"
                             disabled>{{ $t('modeler.common.clear') }}</el-button>
                  <el-button type="text"
                             v-else
                             @click="delAllUserData">{{ $t('modeler.common.clear') }}</el-button>
                </div>
              </div>
              <ul style="height: 314px">
                <vuedraggable :disabled="audit_model !== 'zjsh'"
                              class="wrapper"
                              v-model="check_user_data">
                  <transition-group>
                    <el-tooltip v-for="(item, index) in check_user_data"
                                :key="item.id"
                                class="item"
                                effect="light"
                      ...
                      <li>
                        <span style="cursor: pointer">
                          ...
                          <i class="el-icon-close"
                             @click="delAppointUserData(item)"></i>
                        </span>
                      </li>
                    </el-tooltip>
                  </transition-group>
        ...
        <div v-if="operateType === 'batch'"
             style="padding: 10px 0;">
          <div>{{$t('modeler.chooseRuleTitle')}}</div>
          <div style="padding: 8px 10px;">
            <el-radio v-model="allEditType"
                      label="additional">{{$t('modeler.chooseRule.additional')}}</el-radio>
            <el-radio v-model="allEditType"
                      label="replace">{{$t('modeler.chooseRule.replace')}}</el-radio>
          </div>
        </div>
      </div>
      <div>
        <div v-if="visible && active_step === 1"
             class="choose-table">
          <div class="cell cell-relative">
            <el-tabs type="border-card"
                     :class="['choose-ul', $i18n.locale === 'en-us'?'':'new-tab-ul']"
                     style="height: 324px"
                     v-model="active_name"
                     @tab-click="handleClick">
              <el-tab-pane :label="$t('modeler.userDocLib')"
                           name="user"></el-tab-pane>
              <el-tab-pane :label="$t('modeler.deptDocLib')"
                           name="department"></el-tab-pane>
              <el-tab-pane :label="$t('modeler.customDocLib')"
                           name="custom"></el-tab-pane>
              <div v-if="active_name === 'user'"
                   class="choose-ul"
                   v-loading="loading">
                <div class="choose-ser-1 no-border-bottom">
                  <el-autocomplete clearable
              ...
              <div v-else
                   v-loading="loading">
                <div class="choose-ser-1 no-border-bottom">
                  <el-autocomplete v-if="show_input_search"
                                   style="width: 100%"
                                   size="mini"
                                   clearable
                                   prefix-icon="el-icon-search"
          ...
          <div class="cell cell-btn"><a class="btn-right"
               @click="addSelectedDocLib"></a></div>
          <div class="cell no-border">
            <div class="choose-ul">
              <div class="head">
                <div v-if="isUpdate"
                     class="left">{{ choose_user_name }}&nbsp;{{ $t('field.auditScope') }}：</div>
                <div v-else
                     class="left">{{ $t('modeler.selected') }}：</div>
                <div class="right">
                  <el-button type="text"
                             v-if="check_doc_lib_data.length==0"
                             disabled>{{ $t('modeler.common.clear') }}</el-button>
                  <el-button type="text"
                             v-else
                             @click="delAllDocLibData">{{ $t('modeler.common.clear') }}</el-button>
                </div>
              </div>
              <ul style="height: 345px;margin-top: 10px">
                <li v-for="(item, index) in check_doc_lib_data"
                    :key="index">
                  <span>
                    {{ item.name }}
                    <i class="el-icon-close"
                       @click="delAppointDocLibData(item)"></i>
                  </span>
                </li>
              </ul>
            </div>
      ...
      <span slot="footer"
            class="dialog-footer">
        <el-button v-if="active_step === 0 && isUpdate"
                   style="min-width: 80px"
                   type="primary"
                   size="mini"
                   :disabled="isCheckUserData"
                   @click="confirm">{{ $t('button.confirm') }}</el-button>
        <el-button v-if="active_step === 0 && !isUpdate"
                   :style="'min-width: 80px;',isCheckUserData?'color:#a6a9ad':''"
                   size="mini"
                   :disabled="isCheckUserData"
                   @click="toNextStep">{{ $t('modeler.common.nextStep') }}</el-button>
        <el-button v-if="active_step === 1 && !isUpdate"
                   style="min-width: 80px"
                   size="mini"
                   :disabled="choose_user_id !== ''"
                   @click="toNextStep">{{ $t('modeler.common.preStep') }}</el-button>
        <el-button v-if="!isUpdate"
                   :style="'min-width: 80px;',(isCheckUserData || isCheckDocLibData)?'color:#a6a9ad':''"
                   size="mini"
                   :disabled="(isCheckUserData || isCheckDocLibData)"
                   @click="confirm">{{ $t('modeler.complete') }}</el-button>
        <el-button size="mini"
                   style="min-width: 80px"
                   @click="close">{{ $t('modeler.common.cancel') }}</el-button>
      </span>
    </el-dialog>
    <warnMsg :warnMsgList="warnMsgList"
             :msgVisible="msgVisible"
             @closeWarnMsg="closeWarnMsg"></warnMsg>
    <StepProgress ref="stepProgress"
                  :percentage.sync="percentage"></StepProgress>
  </div>
</template>
<script>
      ...
      return new Promise((resolve) => {
        ...
        checkStrategy(docLibList, _this.procDefId).then(existDocList => {
          _this.check_doc_lib_data.forEach((item) => {
            ...
            if (find !== undefined) {
              const warnMsg = item.name + _this.$i18n.tc('modeler.strategyWarnMsgFix') + ' ' + _this.getDocTypeName(docType) + ' ' + _this.$i18n.tc('modeler.strategyWarnMsg')
              _this.warnMsgList.push(warnMsg)
              if (_this.checkStrategyResult) {
