  ...
  <div>
    <div class="node-wrap" v-if="nodeConfig.type !== 4 && nodeCustomClass(nodeConfig)">
      <div class="node-wrap-box del-node-class" :class="highlightClass" :style="{width: viewProcess ? '300px' : '240px',cursor: viewProcess ? 'grab' : 'pointer'}">
        <div
          :class="'process-node ' + (nodeConfig.error ? 'process_node_error_tip' : '')"
          ...
          <div class="title">
            <el-tooltip
              :content="showNodeName(nodeConfig)"
              placement="top"
              effect="light"
              :disabled="tooltipDisabled">
              <span style="overflow:hidden;text-overflow:ellipsis;white-space:pre;word-break:keep-all;" @mouseenter="checkShowTooltip">{{ showNodeName(nodeConfig) }}</span>
            </el-tooltip>
            <i class="el-icon-close"
               style="right: 10px;position: absolute"
               v-if="nodeConfig.type !== 0 && nodeList.length > 2 && !viewProcess"
               @click="delNode()"/>
          </div>
            ...
            <div class="text" v-if="nodeConfig.type === 1">
              <div v-if="nodeConfig.viewAuditors">
                <template v-if="nodeConfig.viewAuditors.strategyType === 'dept_auditor'">
                  <span style="margin-right: 20px">{{ $t('deptAuditorRule.linkDeptAuditorName') }}</span>
                </template>
                <template v-else-if="nodeConfig.viewAuditors.strategyType === 'multilevel'">
                  <span style="margin-right: 20px">{{ $t('deptAuditorRule.linkMultilevelName') }}</span>
                </template>
                <template v-else-if="nodeConfig.viewAuditors.strategyType === 'excuting_auditor' && tenantId === 'as_workflow'">
                  <span style="margin-right: 20px">{{ $t('deptAuditorRule.ownerAuditorName') }}</span>
                </template>
                <template v-else>
                  ...
                  <el-tooltip
                    class="item"
                    effect="light"
                    :content="nodeConfig.viewAuditors.auditorNames"
                    placement="top"
                    :disabled="tooltipDisabled"
                    v-if="nodeConfig.viewAuditors.auditorNames !== ''">
                    <span @mouseenter="checkAuditorShowTooltip">{{ nodeConfig.viewAuditors.auditorNames | ellipsis(22) }}</span>
                  </el-tooltip>
                  <span v-else>
            ...
            <div class="text" style="text-overflow:inherit;white-space:normal">
              <template v-for="(item, index) in viewAuditors">
                <span :key="index" v-if="item.actDefId === nodeConfig.nodeId">
                  ...
                  <template v-else-if="item.strategyType === 'excuting_auditor' && tenantId === 'as_workflow'">
                    <span style="margin-right: 20px">{{ $t('deptAuditorRule.ownerAuditorName') }}</span>
                  </template>
                  <template v-else>
                    ...
                    <el-tooltip
                      class="item"
                      effect="light"
                      :content="item.auditorNames"
                      :enterable="false"
                      placement="top"
                      v-if="item.auditorNames !== ''"
                      :disabled="tooltipDisabled"
                      style="width: 800px"
                    >
                      <span @mouseenter="checkAuditorShowTooltip">{{ item.auditorNames | ellipsis(35) }}</span>
                    </el-tooltip>
                    <span v-else>
          ...
          <div class="error_tip">
            ...
            <span>{{ $t('sync.settingNodeStrategyTips') }}</span>
          </div>
        </div>
      </div>
      <addNode
...
export default {
  ...
  methods: {
    ...
    setFirstNodeNoAuditorType () {
      this.$emit('setFirstNodeNoAuditorType')
    },
    /**
     * 删除节点
     */
    delNode () {
      const _this = this
      _this.$confirm(_this.$i18n.tc('processCenter.deleteNodeTips'), '', {
        ...
        _this.$emit('setFirstNodeNoAuditorType')
      }).catch(() => { })
    },
    /**
     * 节点自定义class
