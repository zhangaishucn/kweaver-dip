<!-- 流程预览，预览流程设置 -->
<template>
  <div class="view-setting-content">
    <div :class="isTemplateView ? 'tjs-box':''">
      <el-button
        v-if="isTemplateView"
      ...
      <div class="fd-nav-content" style="position:inherit;cursor:grab;" >
        <section class="dingflow-design" :style="{overflowX: 'hidden',background:'inherit',height: `${$store.state.app.previewBox.height -60}px`}">
          <div class="forbidden" v-if="nodeConfig.nodeId" v-drag="{thisVue}" :style="'height:' + siteHeight + 'px;transform: translate(' + siteX + 'px,' + siteY + 'px); '">
            <div id="box-scale" :style="'transform: scale('+nowVal/100+'); transform-origin: 50% 0px 0px;background:inherit'" class="box-scale">
              <nodeWrap
      ...
      <div class="zoom" :style="{background:'transparent',bottom:isTemplateView?'70px':'16px'}">
        <div :class="'zoom-out'+ (nowVal==50?' disabled':'')" @click="zoomSize(1)"></div>
        <span>{{nowVal}}%</span>
        <div :class="'zoom-in'+ (nowVal==200?' disabled':'')" @click="zoomSize(2)"></div>
      </div>
    </div>
    <div v-if="isTemplateView" style="position: absolute; bottom: 0; background: #fff; width: 100%; padding: 15px;  box-sizing: border-box; text-align: right">
      <el-button
...
export default {
  ...
  created() {
    ...
    this.siteHeight = this.$store.state.app.previewBox.height - 60
  },
  methods: {
    type_name(type) {
      const map = {
