...
@Api(tags = "流程模型")
@Slf4j
public class ProcessModelRest  extends BaseRest {
	...
	public ProcessModelVO getProcessDefModel(@PathVariable String id) {
		...
		if (processDefModel.getDocShareStrategyList() != null) {
	...
	@ApiImplicitParams({ @ApiImplicitParam(name = "processDeploymentDTO", value = "流程建模部署对象"),
			@ApiImplicitParam(name = "type", value = "类型；new：生成新版本，update：更新") })
	...
	public ProcessDeploymentVO deployment(@Valid @RequestBody ProcessDeploymentDTO processDeploymentDTO,
			@RequestParam @ArrayValuable(values = { "new", "update" }, message = "文档类型不正确") String type) {
		String userId = this.getUserId();
		return ProcessDeploymentVO.builder()
				.id(processDefinitionService.deployProcess(processDeploymentDTO, type, userId)).build();
	}
