...
import com.aishu.doc.audit.vo.DocAnonymousShareApply;
...
/**
 * @description 共享给任意用户的申请-消息
 * @author ouandyang
 */
@Slf4j
@Component(value = NsqConstants.CORE_AUDIT_SHARE_ANONYMOUS_APPLY)
public class DocAnonymousShareReceiver implements MessageHandler, MessageHandleExecutor {
    ...
    public void handler(java.lang.String msg) {
        if (log.isDebugEnabled()) {
            log.debug("共享给任意用户的申请-事件监听处理类正在处理...");
        }
        if (StringUtils.isEmpty(msg)) {
			return;
		}

        inBoxService.addInBoxMessage(NsqConstants.CORE_AUDIT_SHARE_ANONYMOUS_APPLY, msg);
    }

    @Override
    public void handleMessage(String msg) {
        try {
            log.info("nsq===接收到共享给任意用户的申请审核消息：{}", msg);
            DocAnonymousShareApply nsqMessage = JSONUtil.toBean(msg, DocAnonymousShareApply.class);
            ...
            log.warn("发起共享给任意用户的申请=={error:{}, msg:{}}", e.getMessage(), new String(msg));
            ...
            log.warn("nsq===处理共享给任意用户的申请消息异常, 业务数据不存在！{message：{}}", new String(msg), e);
        } catch (IllegalArgumentException e) {
            log.warn("nsq===处理共享给任意用户的申请消息异常, 申请已存在！{message：{}}", new String(msg), e);
        }  catch (Exception e) {
            SysLogUtils.insertSysLog(SysLogBean.TYPE_ERROR, e.getMessage(), e, msg);
            log.warn("nsq===处理共享给任意用户的申请消息异常！{message：{}}", new String(msg), e);
            throw e;
        } finally {
		}
    }
