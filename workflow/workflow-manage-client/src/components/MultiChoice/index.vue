<template>
  <div class="gol-search" :class="isFocus?'is-focus':''" style="max-width: 480px;min-width: 200px">
    <div class="cell-icon"><i class="el-icon-search" /> </div>
    <div class="cell-text" style="max-width: 312px">
      <template v-for="(item,index) in values">
        ...
        <div :key="item.value" class="list">
          <span :title="item.value" style="width:110px;overflow:hidden;text-overflow:ellipsis;white-space:pre;word-break:keep-all;float: left;">{{ item.value }}</span>
          <a class="close" @click="values.splice(index,1)">x</a>
        </div>
        </input>
      </template>
      <input id="last-input" ref="last" maxlength="0" class="list-input" @focus="isFocus=true">
    </div>
    <div class="cell-input" style="width: 80px">
      <el-autocomplete
         v-model="value1"
         class="inline-input"
         style="width: 80px"
