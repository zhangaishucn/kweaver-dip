...
import com.aishu.doc.audit.vo.DocFlowApply;
 ...
 */
@Slf4j
@Component(value = NsqConstants.CORE_AUDIT_FLOW_APPLY)
public class DocFlowReceiver implements MessageHandler, MessageHandleExecutor {
    ...
    public void handler(java.lang.String msg) {
        if (log.isDebugEnabled()) {
            log.debug("文档流转申请事件监听处理类正在处理...");
        }
        if (StringUtils.isEmpty(msg)) {
            return;
        }

        inBoxService.addInBoxMessage(NsqConstants.CORE_AUDIT_FLOW_APPLY, msg); 
    }

    @Override
    public void handleMessage(String msg) {
        try {
            log.info("nsq===接收到文档流转共享审核消息：{}", msg);
            DocFlowApply nsqMessage = JSONUtil.toBean(msg, DocFlowApply.class);
            ...
            log.warn("发起文档流转申请=={error:{}, msg:{}}", e.getMessage(), new String(msg));
            ...
            log.warn("nsq===处理文档流转审核消息异常！{message：{}}", new String(msg), e);
        } catch (NullPointerException e) {
            log.warn("nsq===处理文档流转审核消息异常, 业务数据不存在！{message：{}}", new String(msg), e);
        } catch (IllegalArgumentException e) {
            log.warn("nsq===处理文档流转审核消息异常, 申请已存在！{message：{}}", new String(msg), e);
        } catch (Exception e) {
            SysLogUtils.insertSysLog(SysLogBean.TYPE_ERROR, e.getMessage(), e, msg);
            log.warn("nsq===处理文档流转审核消息异常！{message：{}}", new String(msg), e);
            throw e;
