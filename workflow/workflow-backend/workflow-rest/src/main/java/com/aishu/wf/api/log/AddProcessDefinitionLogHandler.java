...
/**
 * @description 处理新建流程定义操作日志类
 * @author hanj
 */
@Component(value = OperationLogConstants.ADD_PROCESS_DEFINITION_LOG)
public class AddProcessDefinitionLogHandler extends AbstractLogHandler implements LogHandler {
    ...
    public OperationLogDTO buildLogMsg(Object[] args) {
        ...
        if( args[3].equals(CommonConstants.TENANT_AS_WORKFLOW)){
    ...
    public String buildMsg(Object[] args) {
        if(WorkFlowContants.RENAME_SHARE_PROC_DEF_KEY.equals(args[1]) && isSecret()){
            return StrUtil.format(i18n.getMessage("createWorkflow"),  "内部授权审核工作流");
        }
        return StrUtil.format(i18n.getMessage("createWorkflow"), args[2]);
    }
    ...
    public String buildExMsg(Object[] args) {
        ...
        String procDefKey = (String) args[1];
                ...
                for (DocShareStrategy docShareStrategy : docShareStrategyList) {
                    ...
                    if(WorkflowConstants.STRATEGY_TYPE.NAMED_AUDITOR.getValue().equals(docShareStrategy.getStrategyType())){
                        ...
                        if(WorkflowConstants.STRATEGY_TYPE.DEPT_AUDITOR.getValue().equals(docShareStrategy.getStrategyType())){
                            builder.append(String.format(i18n.getMessage("mode")+"“%s”，"+i18n.getMessage("deptApproverRule")+"“%s”，"+i18n.getMessage("continueUntil")+"“%s”，"+i18n.getMessage("noApprover")+"“%s”，"+i18n.getMessage("multipleApprovers")+"“%s”；", WorkflowConstants.STRATEGY_TYPE.getName(WorkflowConstants.STRATEGY_TYPE.NAMED_AUDITOR.getValue()),
                            ...
                            builder.append(String.format(i18n.getMessage("approverMode")+"“%s”，"+i18n.getMessage("deptApproverRule")+"“%s”，"+i18n.getMessage("end")+"“%s”，"+i18n.getMessage("isNotMatched")+"“%s”，"+i18n.getMessage("multipleAuditors")+"“%s”；", WorkflowConstants.STRATEGY_TYPE.getName(WorkflowConstants.STRATEGY_TYPE.NAMED_AUDITOR.getValue()),
                                    ruleName, levelName, noAuditorType, WorkflowConstants.AUDIT_MODEL.getName(docShareStrategy.getAuditModel())));
                        }
                    }
                }
